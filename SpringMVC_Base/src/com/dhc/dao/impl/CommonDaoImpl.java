...
public class CommonDaoImpl extends BaseDaoImpl implements CommonDao {
	...
	public List<MenuItem> getMenuByUserId(String userId) {
		...
		sb.append("FROM HL_FUNC FUNC  ");
		sb.append("INNER JOIN HL_MENU SND ON FUNC.MENU_ID = SND.MENU_ID ");
		sb.append("INNER JOIN HL_MENU FST ON SND.PARENT_MENU_ID = FST.MENU_ID ");
		...
		sb.append("INNER JOIN HL_MENU SND ON FUNC.MENU_ID = SND.MENU_ID ");
		sb.append("INNER JOIN HL_MENU FST ON SND.PARENT_MENU_ID = FST.MENU_ID ");
		sb.append("INNER JOIN HL_USER_FUNC USRFUNC ON FUNC.FUNC_ID = USRFUNC.FUNC_ID AND USER_ID = :userId ");
		sb.append("UNION ALL ");
		sb.append("SELECT FST.MENU_ID FST_MENU_ID, FST.MENU_NAME FST_MENU_NAME,  SND.MENU_ID SND_MENU_ID, SND.MENU_NAME SND_MENU_NAME, FUNC.FUNC_ID, FUNC.FUNC_NAME, ");
		sb.append("FUNC.REAL_PATH,  FST.SORT_NO FST_SORT_NO, SND.SORT_NO SND_SORT_NO, FUNC.SORT_NO FUNC_SORT_NO ");
		sb.append("FROM HL_USER USR ");
		sb.append("INNER JOIN HL_USER_ROLE USRR ON USR.USER_ID = USRR.USER_ID ");
		sb.append("INNER JOIN HL_ROLE_FUNC RFUNC ON USRR.ROLE_ID = RFUNC.ROLE_ID ");
		sb.append("INNER JOIN HL_FUNC FUNC ON FUNC.FUNC_ID = RFUNC.FUNC_ID ");
		sb.append("INNER JOIN HL_MENU SND ON FUNC.MENU_ID = SND.MENU_ID ");
		sb.append("INNER JOIN HL_MENU FST ON SND.PARENT_MENU_ID = FST.MENU_ID ");
